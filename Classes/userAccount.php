...
class userAccount
{
   ...
   //  validation sign in && sign up and serverside work 
      ...
      if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['singup123'])) {
         ...
         if (isset($correctSfistName) && isset($correctSsurName) && isset($correctSemail_Phn) && isset($correctSpass) && isset($correctSCpass) && isset($correctSbirth_day) && isset($correctSbirth_month) && isset($correctSbirth_year) && isset($correctSGender)) {
            ...
            if (!$insert) {
               ...
               $search_query = dataBaseInput::$connection->query("SELECT * FROM `all users` WHERE  `email_or_mobile` = '$correctSemail_Phn'");
               $fetch_request = $search_query->fetch_assoc();
               $role = $fetch_request['role'];

               //  sing up and kicked into the index.php page
               $_SESSION['all users'] = ["First_Name" => $correctSfistName, "Surname" => $correctSsurName, "email_or_mobile" => $correctSemail_Phn, "password" => $correctSpass, "Birth_day" => $correctSbirth_day, "Birth_month" => $correctSbirth_month, "Birth_year" => $correctSbirth_year, "gender" => $correctSGender, "role" => $role];
            }
         }
      }
